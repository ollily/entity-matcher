 ...
 * <li>matchesAllPropertiesExcluding validates all properties of the given
 * instance excluding the named properties</li>
...
public class EntityMatcher<T> extends TypeSafeMatcher<T> {
    ...
    /**
     * Returns Matcher that checks, if every single property value of a given
     * instance is equal to the properties of an other instance.
     *
     * <em>Note</em>: this implementation is very similar to
     * org.hamcrest.Matchers.samePropertyValuesAs(Person). Differences to the
     * Hamcrest Matcher: while the hamcrest matcher only reports the first property
     * value that does not match, this implementation do return a complete list.
     *
    ...
    /**
     *
     * Returns Matcher that checks, if the valud of every specified property of a
     * given instance is equal to the specified properties of an other instance.
     *
     * @param <T>           type of class this matcher should be created for
     * @param expected      an instance that defines the expected property values
     * @param propertyNames an array containing the names of the properties that
     *                      should be validated
     * @return the configured matcher
     */
    public static <T> EntityMatcher<T> matchesSpecifiedProperties(T expected, String... propertyNames) {
        return new EntityMatcher<>(expected, MatcherType.INCLUDE_NAMED_FIELDS, propertyNames);
    }

    /**
     * Returns Matcher that checks, if every single property excluding the specified
     * properties of a given instance is equal to the properties of an other
     * instance.
     *
     * @param <T>           type of class this matcher should be created for
     * @param expected      an instance that defines the expected property values
     ...
     * @return the configured matcher
     */
    public static <T> EntityMatcher<T> matchesAllPropertiesExcluding(T expected, String... propertyNames) {
    ...
    protected void describeMismatchSafely(T actual, Description mismatchDescription) {
        if (!invalidProperties.isEmpty()) {
            int maxLengthOfPropertyName = invalidProperties.stream().mapToInt(InvalidProperty::getLengthOfPropertyName).max().getAsInt();

            mismatchDescription.appendText(format("got entity with %d invalid values", invalidProperties.size()));

            mismatchDescription.appendText(Arrays.toString(invalidProperties.stream().map(p -> p.getMessage(maxLengthOfPropertyName)).toArray()));
        }

        mismatchDescription.appendText("\n************\n Details:\n").appendText("\t\tActual properties: ").appendText(descriptionOf(actual))
                .appendText("\n\t\tExpected properties:").appendText(descriptionOf(expected));
    }
    ...
    protected boolean matchesSafely(T actual) {
        for (String fieldName : fieldNames) {
            ...
            if (!ObjectUtils.equals(actualValue, expectedValue)) {
                invalidProperties.add(new InvalidProperty(fieldName, expectedValue, actualValue));
            }
        }
    ...
    private String descriptionOf(T object) {
        ToStringBuilder toStringBuilder = new ToStringBuilder(object, ToStringStyle.SHORT_PREFIX_STYLE);
        for (String fieldName : fieldNames) {
    ...
    enum MatcherType {
        EXCLUDE_NAMED_FIELDS, INCLUDE_NAMED_FIELDS
    }
}
...
class InvalidProperty {
    ...
    public String getMessage(int maxLengthOfPropertyName) {
        return format("\n\t-->%s\t(expected:%s, actual:%s)", padRight(fieldName, maxLengthOfPropertyName), expected, actual);
    }
